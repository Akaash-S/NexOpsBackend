...
6. **First-Class Postmortem Management (`/incidents/{id}/postmortem`)**: First-class incident postmortem authoring (draft auto-creation, candidate cause root-cause pre-filling, debounced auto-save, publishing), ungated across all tenant workspaces.
7. **Workspace Feature Flag Guard (`show_extended_navigation`)**: Security dependency (`app/core/security.py:verify_extended_navigation`) gating sidebar navigation items (`/analytics/*`, `/events`, `/automation/*`) behind a workspace tenant flag (default `False`).
